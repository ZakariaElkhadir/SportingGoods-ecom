...
const CategoryCard: React.FC<CategoryCardProps> = ({ name, colors, price, discount, image, logo }) => {
  return (
    
        <div className='bg-white rounded-lg shadow-lg w-96 p-6 m-4 transition-transform hover:scale-105'>
            <div className='relative mb-6'>
                <div className='flex justify-center'>
                    <img src={image} className='w-64 h-64 object-contain rounded-md' alt={name} />
                </div>
                <img src={logo} className='absolute top-2 left-2 w-12 h-12 object-contain' alt="logo" />
            </div>
